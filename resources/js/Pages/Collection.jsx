          ...
          <div className='flex flex-col gap-2 text-sm font-light text-gray-700'>
            {['Men', 'Women', 'Kids'].map((value, index) => (
              <label key={index} className='flex gap-2'>
                <input className='w-3' type='checkbox' value={value} onChange={(e) => toggleFilter(e.target.value, category, setCategory)} />
                {value}
              </label>
            ))}
          </div>
        ...
        <div className={`border border-gray-300 pl-5 py-3 mt-6 ${showFilter ? '' : 'hidden'} sm:block`}>
          ...
          <div className='flex flex-col gap-2 text-sm font-light text-gray-700'>
            {['Topwear', 'Bottomwear', 'Winterwear'].map((value, index) => (
              <label key={index} className='flex gap-2'>
                <input className='w-3' type='checkbox' value={value} onChange={(e) => toggleFilter(e.target.value, subCategory, setSubCategory)} />
                {value}
              </label>
            ))}
          </div>
        ...
        <div className={`border border-gray-300 pl-5 py-3 mt-6 ${showFilter ? '' : 'hidden'} sm:block`}>
          ...
          <div className='flex flex-col gap-2 text-sm font-light text-gray-700'>
            {['Black', 'White', 'Blue', 'Red', 'Green'].map((value, index) => (
              <label key={index} className='flex gap-2'>
                <input className='w-3' type='checkbox' value={value} onChange={(e) => toggleFilter(e.target.value, color, setColor)} />
                {value}
              </label>
            ))}
          </div>
