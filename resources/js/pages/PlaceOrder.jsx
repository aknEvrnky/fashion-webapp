        ...
        <div className='mt-12'>
          ...
          <div className='flex gap-3 flex-col lg:flex-row'>
            <div onClick={() => setMethod('card')} className='flex items-center gap-3 border p-2 px-3 cursor-pointer'>
              <p className={`w-4 h-4 border rounded-full ${method === 'card' ? 'bg-green-400' : ''}`}></p>
              <p className='text-sm text-gray-600'>Credit / Debit Card (Visa / Mastercard)</p>
            </div>

            <div onClick={() => setMethod('bank')} className='flex items-center gap-3 border p-2 px-3 cursor-pointer'>
              <p className={`w-4 h-4 border rounded-full ${method === 'bank' ? 'bg-green-400' : ''}`}></p>
              <p className='text-sm text-gray-600'>Bank Transfer / QR Payment</p>
            </div>
